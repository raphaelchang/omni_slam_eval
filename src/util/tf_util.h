...
public:
    template <typename T>
    static Matrix<T, 3, 1> CameraFrameToWorldFrame(Matrix<T, 3, 1> cameraFramePt)
    {
    ...
    template <typename T>
    static Matrix<T, 3, 1> WorldFrameToCameraFrame(Matrix<T, 3, 1> worldFramePt)
    {
    ...
    template <typename T>
    static Matrix<T, 3, 4> InversePoseMatrix(Matrix<T, 3, 4> poseMat)
    {
    ...
    template <typename T>
    static Matrix<T, 3, 3> GetRotationFromPoseMatrix(Matrix<T, 3, 4> poseMat)
    {
        return poseMat.template block<3, 3>(0, 0);
    }
    template <typename T>
    static Matrix<T, 3, 1> TransformPoint(Matrix<T, 3, 4> tf, Matrix<T, 3, 1> pt)
    {
        Matrix<T, 4, 1> pt_h = pt.homogeneous();
        return tf * pt_h;
    }
    template <typename T>
    static Matrix<T, 3, 1> RotatePoint(Matrix<T, 3, 3> rot, Matrix<T, 3, 1> pt)
    {
        return rot * pt;
    }
    template <typename T>
    static Matrix<T, 3, 4> QuaternionTranslationToPoseMatrix(Quaternion<T> quat, Matrix<T, 3, 1> translation)
    {
        Matrix<T, 3, 4> pose;
